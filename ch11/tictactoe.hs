...
bestmove' g p = do
  bestmoveIndex <- randomRIO (0, length bestmoves - 1) 
...
bestmove g p = head [g' | (depth', Node (g',p') _) <- ts', p' == best, depth' == minDepth]
  where 
    tree = prune depth (gametree g p)
    Node (_,best) ts = minimax tree
    depths = map maxDepth ts
...
play' g p
   ...
   | p == X   = do putStr "Player X is thinking... "
                   let nextMove = bestmove g p
                   (play $! nextMove) (next p)
